...
namespace RulesEngine.UnitTest
{
    ...
    public class OriginalIssueTest
    {
        ...
        public async Task OriginalIssue_RuleChainingWithJsonConfig_ShouldWork()
        {
            ...
            var workflows = JsonSerializer.Deserialize<List<Workflow>>(ruleJson);
            ...
            var engine = new RulesEngine(workflows.ToArray(), new ReSettings { EnableScopedParams = true });
            ...
            Assert.True(results[0].IsSuccess, $"EvaluationExpression should pass but failed: {results[0].ExceptionMessage}");
            ...
            Assert.True(results[1].IsSuccess, $"VerificationExpression should pass but failed: {results[1].ExceptionMessage}");
            ...
            Assert.True(results[2].IsSuccess, $"ActionExpression should pass but failed: {results[2].ExceptionMessage}");
